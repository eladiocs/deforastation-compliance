...
watch(
  ...
  (parcel) => {
    if (parcel) {
      name.value = parcel.name
      clientName.value = parcel.client_name ?? ''
      commodity.value = parcel.commodity ?? ''
    }
  },
  { immediate: true }
)
...
<template>
  <div v-if="parcel" class="fixed inset-0 z-[2000] flex items-center justify-center bg-black/40 px-4">
    <div class="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
      <h2 class="text-lg font-semibold text-gray-900">Editar parcela</h2>
